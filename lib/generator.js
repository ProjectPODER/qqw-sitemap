...
function buildSitemapIndex(files) {
    ...
    files.map(file => {
        lines.push('<sitemap><loc>https://www.quienesquien.wiki/' + file.filename + '</loc><lastmod>' + today + '</lastmod></sitemap>');
    });
...
function buildSubSitemap(slug, items) {
    ...
    items.map( (item, i) => {
        let url = '<loc>https://www.quienesquien.wiki/' + slug + '/' + item.id + '</loc>';
        let lastmod = '';
